...
static int sunxi_pck600_probe(struct platform_device *pdev)
{
	...
	/* The power-controller node may live inside a syscon without a
	 * ranges property, which makes its own reg untranslatable. In
	 * that case fall back to the parent node's reg (the syscon's
	 * MMIO window) which the power controller shares.
	 */
	base = devm_of_iomap(dev, dev_of_node(dev), 0, NULL);
	if (IS_ERR(base) && PTR_ERR(base) == -EINVAL) {
		struct device_node *parent_np = of_get_parent(dev_of_node(dev));

		base = devm_of_iomap(dev, parent_np, 0, NULL);
